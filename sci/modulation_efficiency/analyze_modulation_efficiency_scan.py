...
def make_mode_energy_plot(name, sims):
    ...
    for idx, mode, color in zip(idxs, modes, COLORS):
        ...
        line_kwargs.append(dict(color=color))

        fills.append(
            (np.array([m[idx] for m in mins]), np.array([m[idx] for m in maxs]))
        )
...
if __name__ == "__main__":
    ...
    # ]
    scans = [
        f"unstim_Q=1e{n}_crit_mixing={m}uW.sims" for n in [4, 5, 6] for m in [0, 1]
    ]
