...
def make_mode_energy_scan_plot(pump_powers, results):
    ...
    fit, _ = opt.curve_fit(lambda t, a, n: a * (t ** n), orders, thresholds, p0=[1, 3])
    ...
    si.vis.xxyy_plot(
        ...
        line_labels=["Simulation", rf"${fit[0]:.3g} \times n^{{{fit[1]:.3g}}}$"],
        y_unit="uW",
        y_label="Pump Power",
        vlines=thresholds,
        legend_on_right=True,
...
def calculate_coupling_quality_factors(
    ...
    return {
        mode: coupling_quality_factor_for_tapered_fiber(
...
if __name__ == "__main__":
    ...
    with LOGMAN as logger:
        ...
        spec_kwargs = dict(
            ...
            mode_intrinsic_quality_factors=dict(
                zip(modes, itertools.repeat(coupling_q[pump_mode]))
            ),
        ...
        print(0.5 * (actual - predicted) / (actual + predicted))
