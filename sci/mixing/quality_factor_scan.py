...
def make_mode_energy_scan_plot(quality_factors, results):
    quality_to_sim = dict(zip(quality_factors, results))
    ...
    for q, mode in enumerate(modes):
        ...
        mode_energy = np.array(
            [
                np.mean(
                    sim.mode_energies(sim.mode_amplitudes_vs_time)[-averaging_index:, q]
                )
                for q_factor, sim in quality_to_sim.items()
            ]
        )
        ...
        output_power = np.array(
            [
                np.mean(
                    sim.mode_output_powers(sim.mode_amplitudes_vs_time)[
                        -averaging_index:, q
                    ]
                )
                for q_factor, sim in quality_to_sim.items()
...
if __name__ == "__main__":
    ...
    with LOGMAN as logger:
        ...
        quality_factors = np.geomspace(1e4, 1e8, 50)
        ...
        pumps = {
            ...
            mixing_mode: raman.pump.RectangularMonochromaticPump(
                power=mixing_power, start_time=200 * u.nsec
            ),
        }
        spec_kwargs = dict(
            material=material,
        ...
        for q in quality_factors:
            spec = raman.FourWaveMixingSpecification(
                name=f"q={q}",
                mode_intrinsic_quality_factors=dict(zip(modes, [1e8, 1e8, q, q])),
                **spec_kwargs,
            )
            specs.append(spec)
        ...
        make_mode_energy_scan_plot(quality_factors, results)
