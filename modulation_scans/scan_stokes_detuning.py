...
def main():
    ...
    pump_mode = modulation.resonators.mock.MockMode(
        ...
        * u.c
        ...
        mode_volume_inside_resonator=si.cluster.ask_for_input(
            "Pump mode volume inside resonator?", default=1e-20, cast_to=float
        ),
    ...
    if fixed_q_or_t == "Q":
        pump_intrinsic_q = si.cluster.ask_for_input(
            "Pump mode intrinsic quality factor?", default=DEFAULT_Q, cast_to=float
        )
        ...
        stokes_intrinsic_q = si.cluster.ask_for_input(
            "Stokes mode intrinsic quality factor?", default=DEFAULT_Q, cast_to=float
        )
    ...
    stokes_detunings = (
        ...
        * np.array(
            si.cluster.ask_for_eval(
                "Stokes mode detunings (in GHz)?", default="np.linspace(-100, 100, 200)"
            )
        )
    )
    ...
    pump = modulation.raman.pump.ConstantMonochromaticPump(
        power=u.uW
        * si.cluster.ask_for_input("Pump power (in uW)?", default=100, cast_to=float)
    )
    ...
    lookback_time = shared.ask_lookback_time(time_step, num_modes=2)
    ...
    for stokes_detuning in stokes_detunings:
        ...
        spec = spec_type(
            ...
            mode_intrinsic_quality_factors={
                ...
                or stokes_mode.omega * stokes_intrinsic_tau / 2,
            },
            mode_coupling_quality_factors={
                pump_mode: pump_coupling_q,
                stokes_mode: stokes_coupling_q
                or stokes_mode.omega * stokes_coupling_tau / 2,
            },
            **base_spec_kwargs,
