...
def more_test():
    ...
    coupling_quality_factors = sorted(ps.parameter_set('_mixing_and_modulated_intrinsic_q'))
    intrinsic_quality_factors = sorted(ps.parameter_set('_mixing_and_modulated_coupling_q'))
    ...
    for (mixing_power, intrinsic_q, coupling_q), sims in tqdm(sims_by_mixing_power.items()):
        x = [s.spec._pump_power for s in sims]
        ys = [[s.mode_energies(s.lookback.mean)[mti[mode]] for s in sims] for mode in modes]
    ...
    with si.vis.FigureManager('test', **PLOT_KWARGS) as fm:
        ...
        for mp in mixing_powers:
            ...
            for idx, ((intrinsic_q, coupling_q), sims) in enumerate(tqdm(sims_by_mixing_power.items())):
                ...
                if col == 0:
                    ax.set_ylabel(
                        rf'$Q_i = 10^{int(np.log10(intrinsic_q))}$',
                        fontsize = 9,
                    )
                if row == 2:
                    ax.set_xlabel(
                        rf'$Q_c = 10^{int(np.log10(coupling_q))}$',
                        fontsize = 9,
                    )
