...
FIG_KWARGS = [
    dict(img_format="png", fig_dpi_scale=6, target_dir=OUT_DIR),  # test png
    dict(img_format="pdf", fig_dpi_scale=6, target_dir=OUT_DIR),  # test pdf
]
...
def make_plot(sim):
    for fig_kwargs in FIG_KWARGS:
        with si.vis.FigureManager(
            sim.name, fig_width=TEXTWIDTH, **fig_kwargs
        ) as figman:
            fig = figman.fig
            ...
            for q in mode_numbers[:-1]:
                ax.plot(
                    angles[-1, q], radii[-1, q], color="black", marker="o", linestyle=""
                )
