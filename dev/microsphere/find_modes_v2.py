...
def plot_modal_equation(wavelengths, l, microsphere, polarization):
    me = find.modal_equation(wavelengths, l, polarization, microsphere)
    ...
    modes = find.find_mode_locations(
        [find.WavelengthBound(min(wavelengths), max(wavelengths))], microsphere
    )
    roots = [mode.wavelength for mode in modes if mode.l == l]
    root_kwargs = [
        {"alpha": 0.9, "linewidth": 1, "color": "magenta"} for _ in range(len(roots))
    ]
...
def find_zeros(order=500, which=spc.ai_zeros):
    ...
    z = opt.root(func, 1.1 * np.ones_like(xi), args=(xi,), jac=jac).x
...
def find_roots(
    ...
    wavelength_with_index_zeros = np.array(
        sorted(wavelength_with_index_zeros, key=lambda x: -x)
    )
    ...
    for (top, bottom) in pairwise(wavelength_with_index_zeros):
        ...
        zero = opt.brentq(find.modal_equation, lower, upper, args=args)
        zeros.append(zero)
...
if __name__ == "__main__":
    ...
    modes = find.find_mode_locations(
        [find.WavelengthBound(950 * u.nm, 1000 * u.nm)], ms
    )
    print(len(modes))
